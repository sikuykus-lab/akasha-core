...
**Сначала паутина** — проверить lego-кубики в brain, не блуждать по проекту вслепую. Не блокировка.

```
sessionStart  →  akash pull
новая задача  →  akash prepare "…"  →  read-skill (если кубики совпали)
работа          →  подходят — из кубиков; нет — с нуля
                →  akash remember  ·  akash record-outcome
конец           →  akash sync
```
